...
/**
 * Evaluate a single lead based on:
 * - Estimated value
 * - Contact recency (freshness)
 * - Priority level
 * - Conversion likelihood
 * - Engagement patterns
 */
function evaluateLead(lead: any): LeadEvaluationMetrics {
  ...
  statusScore = statusMap[lead.status] || 10
  ...
  const overallRank = Math.round(
    valueScore * 0.4 + seriousnessScore * 0.3 + actionScore * 0.3
  )
  ...
  let recommendation: "high-priority" | "medium-priority" | "low-priority" =
    "low-priority"
